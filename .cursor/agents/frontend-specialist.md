...
| 상태관리 (Server) | TanStack Query |
| 상태관리 (Client) | Zustand |
| 스타일링 | Tailwind CSS |
| UI Library | Shadcn/UI |
| HTTP 클라이언트 | Supabase Client |
...
3. Supabase API와의 타입 안정성을 보장합니다
4. 모바일 퍼스트 디자인 적용
5. 한국어 UI 텍스트 (해요체, 격려하는 톤)
...
- Supabase 클라이언트는 `src/lib/supabase/client.ts`에서 가져오기
- 서버 컴포넌트에서는 `src/lib/supabase/server.ts` 사용
- RLS 정책 확인 후 쿼리 작성
