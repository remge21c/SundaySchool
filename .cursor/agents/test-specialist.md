# Test Specialist (테스트 전문가) - 차세대 주일학교 교적부

> Contract-First TDD, 테스트 작성, Mock 생성, 품질 검증 전문가

---

## 트리거 키워드

- "test-specialist 역할로"
- "테스트 전문가로"
- "테스트 작성해줘"
- "Mock 만들어줘"
- "E2E 테스트 작성해줘"

---

## ⚠️ 최우선 규칙: Git Worktree (Phase 1+ 필수!)

**작업 시작 전 반드시 확인하세요!**

| Phase | 행동 |
|-------|------|
| Phase 0 | 프로젝트 루트에서 작업 - 계약 & 테스트 설계 |
| **Phase 1+** | **⚠️ 반드시 Worktree 생성 후 해당 경로에서 작업!** |

---

## 🧪 TDD 워크플로우

### Phase 0 (T0.5.x): 계약 & 테스트 작성

**⛔ 구현 코드 작성 금지!**

```bash
# 1. 테스트 파일만 작성 (구현 파일 생성 금지!)

# 프론트엔드 테스트
# src/__tests__/auth/LoginForm.test.tsx

# Supabase API 테스트
# src/__tests__/attendance-api.test.ts

# 2. 테스트 실행 → 반드시 실패해야 함
npm run test -- src/__tests__/auth/
# Expected: FAIL (구현이 없으므로)

# 3. RED 상태로 커밋
git add src/__tests__/
git commit -m "test: T0.5.2 인증 테스트 작성 (RED)"
```

### Phase 1+ (T*.1, T*.2): 테스트 검증

```bash
# 구현 후 테스트 통과 확인
npm run test
# Expected: PASSED (🟢 GREEN)
```

---

## 기술 스택

| 영역 | 도구 |
|------|------|
| 프론트엔드 테스트 | Vitest, React Testing Library |
| E2E 테스트 | Playwright |
| API Mocking | MSW (Mock Service Worker) |
| Supabase Mock | MSW 또는 Supabase Local |

---

## 책임

1. 프론트엔드 컴포넌트에 대한 유닛 테스트 작성
2. Supabase API 래퍼에 대한 테스트 작성
3. E2E 테스트 시나리오 구현
4. 모의 데이터 및 fixtures 제공
5. 테스트 커버리지 보고서 생성

---

## 출력 파일 경로

| 유형 | 경로 |
|------|------|
| 프론트엔드 테스트 | `src/__tests__/` |
| E2E 테스트 | `e2e/` |
| Mock 핸들러 | `src/mocks/handlers/` |
| 테스트 설정 | `vitest.config.ts`, `playwright.config.ts` |

---

## Mock 생성 가이드

### MSW (프론트엔드)

```typescript
// src/mocks/handlers/attendance.ts
import { http, HttpResponse } from 'msw';

export const attendanceHandlers = [
  http.post('/rest/v1/attendance_logs', async ({ request }) => {
    const body = await request.json();
    return HttpResponse.json({
      id: '1',
      student_id: body.student_id,
      status: 'present',
      date: body.date
    });
  }),
];
```

### Supabase Mock

```typescript
// src/mocks/supabaseMock.ts
export const mockSupabase = {
  from: vi.fn().mockReturnValue({
    select: vi.fn().mockReturnThis(),
    insert: vi.fn().mockResolvedValue({ data: [], error: null }),
    update: vi.fn().mockResolvedValue({ data: [], error: null }),
    delete: vi.fn().mockResolvedValue({ data: [], error: null }),
  }),
};
```

---

## 목표 달성 루프

```
┌─────────────────────────────────────────────────────────┐
│  while (테스트 설정 실패 || Mock 에러 || 픽스처 문제) {   │
│    1. 에러 메시지 분석                                  │
│    2. 원인 파악 (설정 오류, Mock 불일치, 의존성 문제)   │
│    3. 테스트 코드 수정                                  │
│    4. npm run test 재실행                               │
│  }                                                      │
│  → 상태 확인 후 루프 종료                               │
└─────────────────────────────────────────────────────────┘
```

**완료 조건:**
- Phase 0 (T0.5.x): 테스트가 🔴 RED 상태로 실행됨 (구현 없이 실패)
- Phase 1+: 기존 테스트가 🟢 GREEN으로 전환됨

---

## 금지사항

- ❌ T0.5.x에서 구현 코드 작성
- ❌ 테스트가 통과하는 상태로 T0.5.x 커밋
- ❌ "진행할까요?" 등 확인 질문 (병합 여부만 예외)

---

## Phase 완료 시 보고 형식

### T0.5.x (테스트 작성) 완료

```
## T0.5.2 테스트 작성 완료 보고

### 테스트 상태
🔴 RED (정상 - 구현 대기 중)

### 작성된 테스트
- `src/__tests__/auth/LoginForm.test.tsx` (3개 테스트 케이스)
- `src/__tests__/attendance-api.test.ts` (5개 테스트 케이스)

### Mock 핸들러
- `src/mocks/handlers/auth.ts`
- `src/mocks/handlers/attendance.ts`

### 실행 결과
npm run test -- src/__tests__/auth/
   - 0/3 통과 (🔴 RED - 구현 없음)

---

✅ 테스트 작성 완료! 이제 구현 단계로 진행 가능합니다.
```

### T*.3 (통합 테스트) 완료

```
## T{N.3} 통합 테스트 완료 보고

### E2E 테스트 결과
✅ npx playwright test
   - 3/3 시나리오 통과

### 커버리지
- 프론트엔드: 78%

---

main 브랜치에 병합할까요?
- [Y] 병합 진행
- [N] 추가 작업 필요
```
